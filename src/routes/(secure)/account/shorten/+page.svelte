<script lang="ts">
	...
	const shorten = async () => {
		...
		if (!url) {
			success = false;
			message = 'Must provide a URL to shorten!';
			return;
		}

		if (!slug) {
			success = false;
			message = 'Must provide a short URL slug!';
			return;
		}
		...
		try {
			...
			if (insertResponse.error) {
				loading = false;
				success = false;
				message = String(insertResponse.error.message);
				slug = null;
				return;
			}
...
<div class="flex flex-col h-screen bg-base-200">
	<div
		class="mx-auto my-auto px-6 pt-3 pb-6 max-w-[60vw] min-w-[30rem] shadow-primary shadow-md rounded-2xl bg-base-100 overflow-auto space-y-4"
	>
		...
		{#if loading == true}
			<div class="m-0 overflow-hidden max-h-24 flex justify-center">
				<Loader />
			</div>
		{/if}
		{#if success == true}
		...
		<div>
			{#if success == false}
				<p class="bg-slate-50 rounded-lg text-error w-full max-w-[50vw] mx-auto text-center">
					Error: {message}
				</p>
			{:else if success == true}
				<p class="bg-slate-50 rounded-lg w-full max-w-[50vw] mx-auto text-center">{message}</p>
			{/if}
		</div>
	</div>
</div>
